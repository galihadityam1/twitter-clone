  ...
  return (
    <View className="flex flex-row align-middle border border-white px-4">
      ...
      <View className=" border-white flex-1 justify-center px-2 flex-col">
        ...
        <Text className="px-2 font-normal text-gray-300 opacity-40">@{profile.username}</Text>
        </View>
        <View className="flex flex-row">
        <Text className="text-white text-xs">Followings {profile.followings.length}</Text>
        <Text className="px-2 text-white text-xs">Followers {profile.followers.length}</Text>
        </View>
      </View>
    </View>
  );
